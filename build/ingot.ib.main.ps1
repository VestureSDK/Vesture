...
task tool-ib-setup -If(-Not (Test-CI-ExecutionEnvironment)) {
    ...
    Write-Step-Start "Setting up dev container settings...";

    if (-Not $env:IS_DEVCONTAINER)
    {
        $devContainerTemplate = "./build/devcontainer/devcontainer-template.json";
        $file = Get-ChildItem -Path $devContainerTemplate;
        if ($file)
        {
            Write-Log Trace "Found dev container template file '${devContainerTemplate}'" -Data  $file;
        }
        else
        {
            Write-Log Error "dev container template file '${devContainerTemplate}' not found";
        }

        $devContainerTemplateContent = Get-Content $devContainerTemplate -Raw;
        Write-File-Content -File $file -Content $devContainerTemplateContent;
        
        $devContainerMount = "${BuildRoot}";
        if ($IsWindows)
        {
            if (Get-Command "wsl" -errorAction SilentlyContinue)
            {
                $devContainerMount = $devContainerMount -Replace "\\", "/";
                $devContainerMount = exec { wsl wslpath -a "${devContainerMount}" }
            }
            else 
            {
                $devContainerMount = $undefined;
            }
        }

        if (-Not $devContainerMount)
        {
            Write-Log Warning (
                "WSL does not appear to be setup on your system.`n" +
                "It is strongly suggest to setup WSL and support for dev containers`n" +
                "in order for consistency between local and CI."
            );
        }
        else 
        {
            $templateKey = "__DevContainerMount__";
    
            Write-Log Trace "Interpolating template key '${templateKey}' with '${devContainerMount}'";
            $devContainerContent = $devContainerTemplateContent -Replace "__DevContainerMount__", $devContainerMount;
    
            $devContainerDirectory = "./.devcontainer";
            New-Directory $devContainerDirectory;
    
            $devContainerFile = "${devContainerDirectory}/devcontainer.json";
            
            Write-Log Trace "Creating file '${devContainerFile}'...";
            $devContainerContent > $devContainerFile;
    
            $file = Get-ChildItem -Path $devContainerFile;
            if ($file)
            {
                Write-Log Trace "Successfully created dev container file '${devContainerFile}'" -Data  $file;
            }
            else
            {
                Write-Log Error "dev container file '${devContainerFile}' not found";
            }
    
            $devContainerContent = Get-Content $file -Raw;
            Write-File-Content -File $file -Content $devContainerContent;
    
            Write-Step-End "Successfully set up dev container settings";
        }
    }
    else
    {
        Write-Step-End "Running in a dev container already, no setup required";
    }
