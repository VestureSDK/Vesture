namespace Crucible.Mediator.Commands
{
    /// <summary>
    /// <para>
    /// The <see cref="ICommand"/> interface defines a contract for commands in your application. 
    /// </para>
    /// <para>
    /// Commands represent actions or operations that trigger state changes in the system, such as creating, 
    /// updating, or deleting data. Commands are typically handled by a handler that implements <c>IRequestHandler{TRequest, TResponse}</c>.
    /// </para>
    /// <para>
    /// When invoked via the <c>IMediator</c>, the appropriate handler is determined and the logic to process 
