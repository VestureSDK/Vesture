...
namespace Crucible.Mediator.Engine.Pipeline
{
    public class DefaultInvocationPipeline<TRequest, TResponse> : IInvocationPipeline<TResponse>
    {
        ...
        {
            var middlewares = new List<IInvocationPipelineItem<TRequest, TResponse>>();
            ...
            foreach (var middleware in _middlewares.OrderBy(m => m.Order))
            {
                ...
                {
                    middlewares.Add((IInvocationPipelineItem<TRequest, TResponse>)middleware);
                }
            }

            Func<IInvocationContext<TRequest, TResponse>, CancellationToken, Task> handler = (ctx, ct) => _handler.HandleAsync(ctx, null, ct);
            Func<IInvocationContext<TRequest, TResponse>, CancellationToken, Task> chain = (ctx, ct) =>
            {
                ...
                return preHandlerMiddleware.HandleAsync((IInvocationContext<object, object>)ctx, (t) => handler.Invoke(ctx, t), ct);
            };
            ...
            for (var i = middlewares.Count - 1; i >= 0; i--)
            {
                ...
                chain = (ctx, ct) => item.HandleAsync(ctx, nextMiddleware, ct);
            }
