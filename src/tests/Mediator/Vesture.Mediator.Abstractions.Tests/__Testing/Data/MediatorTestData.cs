using Vesture.Mediator.Abstractions.Tests.Data.Internal;
using Vesture.Mediator.Commands;
using Vesture.Mediator.Events;
using Vesture.Mediator.Invocation;
using Vesture.Mediator.Mocks.Commands;
using Vesture.Mediator.Mocks.Events;
using Vesture.Mediator.Mocks.Invocation;
using Vesture.Mediator.Mocks.Requests;
using Any = object;

namespace Vesture.Mediator.Abstractions.Tests.Data
{
    public static class MediatorTestData
    {
        ...
        public static IEnumerable<ContractRequestResponseMediatorRequestResponseTestData> Get_RequestResponseMediatorRequestResponse_IsApplicable_All(
            Func<ContractRequestResponseMediatorRequestResponseTestData, bool>? predicate = null
        )
        {
            ...
            foreach (var requestResponse in allRequestResponses)
            {
                ...
                if (
                    requestResponse.Request is MockContract
                    && requestResponse.Request.GetType() != typeof(MockContract)
                )
                {
                    data.Add(
                        new()
                ...
                if (
                    requestResponse.Response is MockContract
                    && requestResponse.Response.GetType() != typeof(MockContract)
                )
                {
                    data.Add(
                        new()
                ...
                if (
                    requestResponse.Response is NoResponse
                    && requestResponse.Response.GetType() != NoResponse.Type
                )
                {
                    data.Add(
                        new()
